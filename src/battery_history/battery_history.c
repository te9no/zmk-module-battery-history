...
#define RECORDING_INTERVAL_MS \
    (CONFIG_ZMK_BATTERY_HISTORY_INTERVAL_MINUTES * 60 * 1000)
...
// minutes have passed
#define MIN_SAME_LEVEL_INTERVAL_SEC \
    (CONFIG_ZMK_BATTERY_HISTORY_INTERVAL_MINUTES * 60 * 4)
...
static void add_history_entry(uint16_t timestamp, uint8_t level) {
    ...
    LOG_DBG(
        "Added battery history entry: timestamp=%u, level=%u, idx=%d "
        "(total=%d, unsaved=%d)",
...
static int save_history(void) {
    ...
    LOG_INF(
        "Saving battery history to flash (count=%d, unsaved=%d, "
        "head_changed=%d)",
    ...
    rc = settings_runtime_set("battery_history/head", &history_head,
                              sizeof(history_head));
    if (rc < 0) {
    ...
    rc = settings_runtime_set("battery_history/count", &history_count,
    ...
    if (first_unsaved_idx >= 0) {
        ...
        LOG_DBG("Incremental save: %d entries starting from idx %d",
                entries_to_save, idx);
...
static bool should_record_entry(uint16_t timestamp, uint8_t level) {
    ...
    LOG_DBG(
        "Skipping record: level unchanged (%d%%), time_diff=%u < threshold=%d",
...
/**
 * Settings load handler - supports both old format (full buffer) and new format
 * (individual entries)
 */
static int battery_history_settings_set(const char *name, size_t len,
    ...
    if (name[0] == 'e') {
        ...
        if (idx >= 0 && idx < MAX_ENTRIES) {
            ...
            return read_cb(cb_arg, &history_buffer[idx],
                           sizeof(struct zmk_battery_history_entry));
        }
    }
 ...
 */
static int battery_history_settings_commit(void) {
    LOG_INF("Battery history loaded: count=%d, head=%d", history_count,
            history_head);
...
static int battery_history_event_listener(const zmk_event_t *eh) {
    const struct zmk_battery_state_changed *bev =
...
static int battery_history_activity_listener(const zmk_event_t *eh) {
    const struct zmk_activity_state_changed *aev =
...
static int battery_history_init(void) {
    ...
    LOG_INF(
        "Max entries: %d, Recording interval: %d minutes, Save level "
        "threshold: %d%%",
        MAX_ENTRIES, CONFIG_ZMK_BATTERY_HISTORY_INTERVAL_MINUTES,
        SAVE_LEVEL_THRESHOLD);
...
int zmk_battery_history_get_entry(int index,
...
int zmk_battery_history_clear(void) {
    ...
    settings_runtime_set("battery_history/head", &history_head,
                         sizeof(history_head));
    settings_runtime_set("battery_history/count", &history_count,
                         sizeof(history_count));
    settings_save();
